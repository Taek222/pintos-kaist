...
void check_address (void *address) {
	...
	/*
		|: 비트 연산자, 왼쪽이 참이든 아니든 상관없이 왼쪽, 오른쪽 둘 다 검사
		||: 논리 연산자, 왼쪽이 참이면 오른쪽 검사 안 함
	*/
...
// find_file_by_fd: 현재 스레드가 읽고 있는 fd를 리턴하는 함수, 1 = STDIN / 2 = STDOUT
struct file *find_file_by_fd (int fd) {
...
int add_file_to_fdt(struct file *file) {
	...
	/*
		fd 테이블의 앞부터 순차적으로 검색, 범위를 넘지 않도록 함
		fd_index는 fdt 인덱스의 마지막 값
		같은 파일을 열더라도 다른 fd를 리턴해야 함 (깃북 참고) 그래서 +1 해주는 것
